...
router.post('/', async (req: Request, res: Response) => {
    ...
    if (bookData) {
        ...
        if (bookData.stock > 0) {
            ...
            return res.status(500).json({message: '书籍库存不足'});
        }
    } else {
        return res.status(500).json({message: '书籍不存在'});
    }

    const obj = await borrow.save();
    return res.status(200).json({data: obj, message: true});
});

router.get('/:id', async (req: Request, res: Response) => {
    const data = await Borrow.findOne({_id: req.params.id}).populate(['book']);
    console.log('data')
    console.log(data)
    if (data) {
